// berkeley_biped_node.cpp
// Berkeley-Humanoid-Lite ROS 2 policy inference node.
//
// Responsibilities:
...
class BerkeleyBipedPolicyNode : public rclcpp::Node
{
public:
    BerkeleyBipedPolicyNode()
    : Node("berkeley_biped_node"),
      env_(ORT_LOGGING_LEVEL_WARNING, "berkeley_biped_policy")
    {
        const std::string package_share_dir =
            ament_index_cpp::get_package_share_directory("berkeley_biped_pkg");
        ...
        if (override_imu_) {
            ...
            imu_subscriber_ = this->create_subscription<sensor_msgs::msg::Imu>(
                ...
                std::bind(&BerkeleyBipedPolicyNode::imu_callback, this, std::placeholders::_1));
        }
        ...
        joint_states_subscriber_ = this->create_subscription<sensor_msgs::msg::JointState>(
            ...
            std::bind(&BerkeleyBipedPolicyNode::joint_state_callback, this, std::placeholders::_1));
        ...
        if (!use_sim_ && require_joint_feedback_age_) {
            joint_feedback_age_subscriber_ =
                this->create_subscription<std_msgs::msg::UInt32MultiArray>(
                    ...
                    std::bind(
                        &BerkeleyBipedPolicyNode::joint_feedback_age_callback,
                        this,
                        std::placeholders::_1));
        }
        ...
        RCLCPP_INFO(
            this->get_logger(),
            "Berkeley policy node initialized: obs[%zu] -> actions[%zu], policy_dt=%.3fs.",
...
private:
    ...
    void publish_policy_status(bool ready, const std::string& reason)
    {
        ...
        status_msg.data = ready ? "READY" : ("NOT_READY: " + reason);
    ...
    void run_policy_and_publish()
    {
        ...
        try {
            ...
            publish_policy_status(true, "ready");
        } catch (const Ort::Exception& error) {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        auto node = std::make_shared<BerkeleyBipedPolicyNode>();
        executor.add_node(node);
        executor.spin();
    } catch (const std::exception& error) {
        std::cerr << "Fatal Berkeley biped policy node error: " << error.what() << std::endl;
        rclcpp::shutdown();
        return 1;
    }
