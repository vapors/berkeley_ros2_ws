#include "bhl_st3215_driver/diagnostics.hpp"
#include "bhl_st3215_driver/joint_map.hpp"
#include "bhl_st3215_driver/servo_bus.hpp"
#include "bhl_st3215_driver/state_buffer.hpp"
#include "bhl_st3215_driver/msg/servo_telemetry.hpp"
...
class ServoDriverNode final : public rclcpp::Node {
 public:
  ServoDriverNode() : Node("bhl_st3215_driver") {
    declareParameters();

    const std::string package_share =
        ament_index_cpp::get_package_share_directory("bhl_st3215_driver");
    ...
    telemetry_pub_ = create_publisher<bhl_st3215_driver::msg::ServoTelemetry>(
    ...
    RCLCPP_INFO(
        get_logger(),
        "Native ST3215 driver starting: port=%s baud=%d joints=%zu bus=%.1fHz command=%.1fHz writes=%s map=%s",
        config.port.c_str(), config.baud, joint_map_.size(), config.bus_rate_hz,
 ...
 private:
  ...
  rclcpp::Publisher<bhl_st3215_driver::msg::ServoTelemetry>::SharedPtr telemetry_pub_;
...
int main(int argc, char** argv) {
  ...
  try {
    rclcpp::spin(std::make_shared<bhl_st3215_driver::ServoDriverNode>());
  } catch (const std::exception& error) {
    RCLCPP_FATAL(rclcpp::get_logger("bhl_st3215_driver"), "%s", error.what());
    rclcpp::shutdown();
    return 1;
  }
  rclcpp::shutdown();
