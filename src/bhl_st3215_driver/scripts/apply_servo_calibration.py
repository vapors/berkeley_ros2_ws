...
def default_source_map() -> Path:
    candidate = Path.home() / "berkeley_ros2_ws" / "src" / "bhl_st3215_driver" / "config" / "servo_map.yaml"
    return candidate
...
def main() -> int:
    ...
    map_path = (
        args.servo_map.expanduser().resolve()
        if args.servo_map
        else default_source_map().expanduser().resolve()
    )
    ...
    if not map_path.is_file():
        ...
        print("Pass --servo-map explicitly.", file=sys.stderr)
        return 2
