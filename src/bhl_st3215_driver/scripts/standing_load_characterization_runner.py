#!/usr/bin/env python3
"""Whole-body standing/crouch load-characterization runner for BHL Track 2.
...
import rclpy
import yaml
from bhl_st3215_driver.msg import ServoTelemetry
...
NUM_JOINTS = 12
DEFAULT_POSE_LIBRARY = Path.home() / '.ros' / 'bhl_standing_poses.yaml'
DEFAULT_CAPTURE_AUDIT_ROOT = Path.home() / '.ros' / 'bhl_standing_pose_capture_audits'
...
def resolve_servo_map(configured: str) -> Path:
    ...
    if get_package_share_directory is not None:
        try:
            candidates.append(
                Path(get_package_share_directory('bhl_st3215_driver'))
                / 'config' / 'servo_map.yaml'
            )
        except Exception:
            pass
    candidates.append(
        Path.home() / 'berkeley_ros2_ws' / 'src' /
...
def resolve_track1_contract(configured: str) -> Path:
    ...
    candidates.append(
        Path.home() / 'berkeley_ros2_ws' / 'src' /
        'bhl_st3215_driver' / 'config' / 'track1_action_contract_v3.yaml'
    )
    for path in candidates:
        if path.is_file():
            return path.resolve()
...
def load_pose_library(path: Path, joints: Sequence[JointConfig]) -> Dict[str, object]:
    ...
    raw.setdefault('robot', 'Berkeley-Humanoid-Lite')
...
class StandingLoadNode(Node):
    ...
    def graph_guard(self) -> Tuple[bool, str, Dict[str, object]]:
        ...
        forbidden = [
            n for n in nodes
            if n.endswith('/berkeley_biped_node')
...
def evaluate_mode(node: StandingLoadNode, joints: Sequence[JointConfig], map_path: Path, args, output_dir: Path) -> int:
    ...
    return 0 if final_status == 'completed' else 5
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--output-root', default=str(Path.home() / 'berkeley_ros2_ws' / 'track2_standing_reports'))
