...
- `servo_identification_runner.py` logs telemetry as the authoritative time-series
...
The package now generates `bhl_st3215_driver/msg/ServoTelemetry`, so rebuild and
...
Launch only the native driver. Do not launch `berkeley_biped_pkg` or the outer PD
...
```bash
ros2 run bhl_st3215_driver servo_identification_runner.py \
